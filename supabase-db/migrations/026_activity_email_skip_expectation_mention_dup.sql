-- Receivers get mention activity mail ("added as a receiver"); skip duplicate
-- changelog "Created a new expectation" mail for them. Talking-point @mentions
-- still use publish changelog only (no mention mail).
...
COMMENT ON FUNCTION public.inled_enqueue_changelog_activity_emails() IS
  'Changelog activity mail; skips author on create; skips receivers on expectation create (mention mail).';

-- Ensure expectation mention mail stays enabled (talking points still skip — publish changelog only).
